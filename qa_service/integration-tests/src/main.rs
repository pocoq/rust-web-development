...
use qa_service_dev::{handle_errors, oneshot, setup_store};
...
async fn main() -> Result<(), handle_errors::Error> {
    dotenv::dotenv().ok();
    let config = qa_service_dev::config::Config::new().expect("Config can't be set");
    ...
    let u = User {
        email: "example_test5@email.com".to_string(),
        password: "cleartext".to_string(),
    };
    let token;
...
async fn post_question(token: Token) {
    ...
    assert_eq!(res.content, q.content);
}
